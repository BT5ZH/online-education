<template>
  <main class="upload" id="file-upload">
    ...
    <div
      class="upload__drag"
      :style="{ 'background-color': borderhover ? '#3d8cff' : '#f9f7f6' }"
    >
      <div
        class="upload__drag__area "
        v-for="(item, index) in files"
        :key="index"
      >
        <img
          class="upload__drag__area--img preview"
          v-bind:ref="'preview' + parseInt(index)"
        />
        <svg class="upload__drag__area--btn" v-on:click="removeFile(item)">
          <use
            xlink:href="../../../assets/img/all.svg#icon-circle-with-cross"
          ></use>
        </svg>
      </div>
    </div>
            ...
            <div class="process__bar__detail">
              ...
              <div class="process__bar__detail__item">
                ...
                <span class="process__item--info">
                  {{ currentLoad[index] | fileSize | fileFormat
                  }}{{ file.size | fileSize }}</span
                >
              </div>
              <div
                class="process__bar__detail--btn"
                @click="uploadBusiness(index)"
              >
                上传
              </div>
            </div>
            <div
              class="process__bar__view"
              id="index"
  ...
  methods: {
    enentDrop: function(e) {
      ...
      for (let i = 0; i < e.dataTransfer.files.length; i++) {
        if (/\.(jpe?g|png|gif|mp4|zip)$/i.test(e.dataTransfer.files[i].name)) {
    ...
    getImagePreviews() {
      for (let i = 0; i < this.files.length; i++) {
        if (/\.(jpe?g|png|gif)$/i.test(this.files[i].name)) {
          ...
          this.duration[i] = 0;
        } else if (/\.(mp4)$/i.test(this.files[i].name)) {
          ...
          setTimeout(() => {
            if (this.duration[i] == null || this.duration == "") {
              ...
              this.duration[i] = document.getElementById(
                "videoPlayer"
              ).duration;
            }
          }, 800);
          this.$nextTick(function() {
            this.$refs["preview" + parseInt(i)][0].src =
              "https://rs-learning-resources.s3.cn-northwest-1.amazonaws.com.cn/public/index/mp4.png";
          });
        } else {
          this.duration[i] = 0;
          this.$nextTick(function() {
            this.$refs["preview" + parseInt(i)][0].src =
              "https://rs-learning-resources.s3.cn-northwest-1.amazonaws.com.cn/public/index/zip.jpeg";
          });
        }
      }
    },
    ...
    getPass() {
      ...
      axios
        ...
        .then((response) => {
          ...
          let AWSConfig = {
            ...data.Credentials,
            path: "homework/resources/" + userId + "",
            id: data.id,
          };
          ...
          const progress = (event) => {
            ...
            this.$set(
              ...
              this.currentIndex,
              this.uploadPercentage[this.currentIndex]
            );
